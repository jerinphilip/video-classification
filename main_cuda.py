from parser import create_parser_cuda as create_parser
...
if __name__ == '__main__':
    ...
    with open(args.cudaout,'w') as cudaoutfile:
        for i, descriptor in enumerate(descriptors_flattened):
            y = map(str,descriptor)
            string = ' '.join(y) + '\n'
            print(i+1, string, end="", file = cudaoutfile)
    print("Done")
    exit(0) 
    # Generate K Means
